...
{block name='module_products'}
  <section class="ps-featuredproducts">
    <div class="module-products container">
      <div class="module-products__split">

        <div class="module-products__intro">
          {if !empty($hbe_fp_title)}
            {include file='components/section-title.tpl' title=$hbe_fp_title}
          {else}
            {include file='components/section-title.tpl' title={l s='Featured products' d='Shop.Theme.Catalog'}}
          {/if}

          {if !empty($hbe_fp_text)}
            <p class="hbe-section-text">{$hbe_fp_text|escape:'html':'UTF-8'}</p>
          {/if}

          <div class="module-products__buttons module-products__buttons--intro">
            {if !empty($hbe_fp_link_text) && !empty($hbe_fp_link_url)}
              <a class="btn btn-primary hbe-section-link" href="{$hbe_fp_link_url|escape:'html':'UTF-8'}">
                {$hbe_fp_link_text|escape:'html':'UTF-8'}
              </a>
            {/if}
          </div>
